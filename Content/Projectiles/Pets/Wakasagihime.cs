...
namespace TouhouPets.Content.Projectiles.Pets
{
    public class Wakasagihime : BasicTouhouPet
    {
        ...
        public override TouhouPetID UniqueID => TouhouPetID.Wakasagihime;
        public override bool DrawPetSelf(ref Color lightColor)
        {
            ...
            for (int i = 0; i < 7; i++)
            {
                Projectile.DrawPet(9, Color.White * 0.5f * mouseOpacity,
                    config with
                    {
                        PositionOffset = new Vector2(0, extraHeight) + new Vector2(Main.rand.Next(-10, 11) * 0.2f, Main.rand.Next(-10, 11) * 0.05f),
                    });
            }
        ...
        private void BlowingBubble()
        {
            ...
            if (Projectile.frame == 3 && Projectile.frameCounter == 0)
            {
                ...
                {
                    Dust bubble = Dust.NewDustPerfect(Projectile.Center + new Vector2(4 * Projectile.spriteDirection, 4)
                        , DustType<WakasagihimeBubble>(), Vector2.Zero);
                    bubble.velocity = new Vector2(Main.rand.NextFloat(1f, 2f) * Projectile.spriteDirection, 0);
                }
            }
